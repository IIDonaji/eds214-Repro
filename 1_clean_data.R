...
# Load in the packages used for this analysis 
library(here)
library(tidyverse)
library(janitor)
library(skimr)
...
combo_chemi_data <- rbind(Q1, Q2, Q3, MPRM) %>% 
  mutate(Sample_Date = ymd(Sample_Date)) %>% # parse dates with year, month, and day
...
stream_water_chemi <- combo_chemi_data %>% 
  pivot_longer(cols = c(k, no3_n, mg, ca, nh4_n), # only want these ions 
               names_to = "ions", # name for ions column
               values_to = "water_conc") %>% # new column name for values 
...
stream_anly <- stream_water_chemi %>% 
  ...
  arrange(sample_date,ions) %>% # arranging for clarity 
  group_by(ions, basins) %>% # only want operation performed on these columns
  mutate(conc_aver = sapply( # creating my concentration average using moving_average FUN.
    sample_date,
    moving_average,
    dates = sample_date,
    conc = water_conc,
...
# created an RDS to save my output for this analysis
